...
import { ManagedUser, UserRole, UserScope, UsersService } from '../users.service';
...
export class UsersPage implements OnInit, OnDestroy {
  ...
  save(): void {
    ...
    const unitId = this.form.scope === 'main' ? null : this.normalizeUuid(this.form.unitId);
  ...
  private fail(error: unknown, message: string): void {
    this.errorMessage = (error as { error?: { message?: string } })?.error?.message || message;
  ...
  get availableUnits(): Unit[] {
    ...
    if (this.form.scope === 'ew') {
      const ewUnits = this.units
        .filter((unit) => this.isUuid(unit.id))
        .filter((unit) => this.isEwUnit(unit));

      return ewUnits.length > 0
        ? ewUnits
        : this.units
            .filter((unit) => this.isUuid(unit.id))
  ...
  private isEwUnit(unit: Unit): boolean {
    const type = unit.type.toLowerCase();
  ...
  private isExplicitBatteryUnit(unit: Unit): boolean {
    const type = unit.type.toLowerCase();
    return type === 'battery' || type.includes('battery') || type.includes('бат');
  }
