...
@Component({
      ...
      <section class="eu-nc-panel" *ngIf="open" aria-label="Центр повідомлень">
        <header class="eu-nc-header">
          ...
          <button
            ...
            (click)="close($event)"
            aria-label="Закрити"
            title="Сховати центр повідомлень"
            data-tip-pos="left"
          >
            ×
          </button>
        ...
        <section class="eu-nc-stats" aria-label="Зведення повідомлень">
          <button
            type="button"
            class="eu-nc-stat"
            [class.active]="mode === 'notifications'"
            (click)="setMode('notifications')"
            title="Показати нові непрочитані події"
          >
            <b>{{ stats.unread }}</b>
            <span>Нові</span>
          </button>
          ...
          <button
            ...
            (click)="setMode('today')"
            title="Показати події поточної доби"
          >
            <b>{{ stats.today }}</b>
            <span>Сьогодні</span>
          </button>
          <button
            ...
            (click)="setMode('journal')"
            title="Відкрити повний журнал подій"
          >
            <b>{{ stats.journal }}</b>
            <span>Журнал</span>
          </button>
        ...
        <section class="eu-nc-controls">
          <nav class="eu-nc-tabs" aria-label="Режим центру повідомлень">
            <button
              ...
              [class.active]="mode === 'notifications'"
              (click)="setMode('notifications')"
              title="Оперативна черга непрочитаних повідомлень"
            >
              Центр
            </button>
            <button
          ...
          <div class="eu-nc-filter-row">
            ...
            <button
              ...
              [class.active]="priorityFilter === 'critical'"
              (click)="setPriorityFilter('critical')"
              title="Показати тільки критичні"
            >
              Крит
            </button>
            <button
            ...
            <button
              ...
              *ngIf="mode === 'notifications'"
              title="Сховати вже переглянуті повідомлення"
            >
              Непрочитані
            </button>
          </div>
          ...
          <div class="eu-nc-action-row">
            <span class="eu-nc-sync" [class.refreshing]="eventFeed.loading$ | async">
              {{ (eventFeed.loading$ | async) ? 'Оновлення' : 'Оновлено' }} ·
              {{ eventFeed.lastSyncedLabel }}
            </span>
            <button
              type="button"
              (click)="expandPriority()"
              *ngIf="filteredGroups.length"
              title="Розкрити критичні та важливі групи"
            >
              Розкрити важливі
            </button>
            <button
              ...
              (click)="collapseAll()"
              *ngIf="filteredGroups.length"
              title="Згорнути всі групи повідомлень"
            >
              Згорнути
            </button>
            <button
              ...
              (click)="markSeen()"
              *ngIf="mode === 'notifications'"
              title="Позначити видимі повідомлення прочитаними"
            >
              Прочитано
            </button>
            <button type="button" (click)="eventFeed.load()" title="Оновити події з сервера">
              {{ (eventFeed.loading$ | async) ? 'Оновлюю…' : 'Оновити' }}
            </button>
            <button
              type="button"
              (click)="resetFilters()"
              *ngIf="hasFilters"
              title="Очистити пошук і фільтри"
            >
              Скинути
            </button>
          </div>
        ...
        <section class="eu-nc-status" *ngIf="eventFeed.loading$ | async">
          <span class="eu-nc-loader"></span>
          <p>Оновлюю події…</p>
        </section>
        <section
          class="eu-nc-status is-error"
          *ngIf="eventFeed.errorMessage && !(eventFeed.loading$ | async)"
        >
          <p>{{ eventFeed.errorMessage }}</p>
          <button type="button" (click)="eventFeed.load()" title="Повторити завантаження подій">
            Повторити
          </button>
        </section>
        ...
        <section class="eu-nc-body">
          <ng-container *ngIf="notificationSections.length; else emptyFeed">
            <section
              class="eu-nc-section"
              *ngFor="let section of notificationSections; trackBy: trackSection"
              [class.is-muted]="section.groups.length === 0"
            >
              ...
              <article
                class="eu-nc-group"
                *ngFor="let group of section.groups; trackBy: trackGroup"
                [ngClass]="[
                  'is-' + group.priority,
                  'tone-' + group.tone,
                  group.unreadCount > 0 ? 'is-unread' : '',
                ]"
              >
                <button
                  type="button"
                  class="eu-nc-group-head"
                  (click)="toggleGroup(group.key)"
                  title="Розкрити групу повідомлень"
                >
                  <span class="eu-nc-priority">{{ priorityLabel(group.priority) }}</span>
                  <span class="eu-nc-group-main">
                    <strong>{{ group.title }}</strong>
                    <small
                      >{{ group.unitName || 'Без підрозділу' }} ·
                      {{ group.latestAt | date: 'dd.MM HH:mm' }}</small
                    >
                    <em>{{ group.summary }}</em>
                  </span>
                ...
                <div class="eu-nc-events" *ngIf="isOpen(group.key)">
                  <button
                    type="button"
                    class="eu-nc-event"
                    *ngFor="let event of group.items; trackBy: trackEvent"
                    [class.is-unread]="eventFeed.isUnread(event)"
                    (click)="openEvent(event)"
                    title="Відкрити повʼязаний екран"
                  >
                    ...
                    <span class="eu-nc-event-main">
                      ...
                      <em
                        >{{ event.actorName || event.actorLogin || 'Система' }} ·
                        {{ eventFeed.getEventActionLabel(event) }}</em
                      >
                    </span>
                  </button>
                </div>
  ...
  styles: [
    `
      ...
      .eu-nc-filter-row button.active {
        color: #dffcf7;
        border-color: rgba(45, 212, 191, 0.42);
        background: rgba(45, 212, 191, 0.1);
      }
      ...
      .eu-nc-body::-webkit-scrollbar {
        width: 8px;
      }
      .eu-nc-body::-webkit-scrollbar-thumb {
      ...
      .eu-nc-section-head div {
        min-width: 0;
      }
      .eu-nc-section-head h3 {
        margin: 0;
        color: #f8fafc;
        font-size: 13px;
        text-transform: uppercase;
      ...
      .eu-nc-group.is-critical {
        border-color: rgba(248, 113, 113, 0.42);
      }
      .eu-nc-group.is-high {
        border-color: rgba(251, 191, 36, 0.32);
      }
      .eu-nc-group.is-unread {
...
export class EventFeedPanelComponent {
  ...
  get notificationSections(): Array<{
    key: string;
    title: string;
    subtitle: string;
    groups: EventGroup[];
      ...
      return [
        {
          ...
          subtitle:
            this.mode === 'today' ? 'Всі події за сьогодні' : 'Останні зафіксовані дії системи',
          groups,
        },
